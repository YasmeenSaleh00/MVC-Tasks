...
namespace Practise_Database_Migration.Controllers
{
    public class CategoriesController : Controller
    {
        ...
        public CategoriesController(SimpleEcommarceDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> Create([Bind("Name,Description")] Category category, IFormFile ImagePath)
        {
            if (ModelState.IsValid)
            {
                if (ImagePath != null)
                {
                    category.ImagePath = await FileHelper.UploadFileAsync(ImagePath, "Categories");
                }
        ...
        public async Task<IActionResult> Edit(int id, [Bind("Name,Description,ImagePath,Id,IsDeleted")] Category category, IFormFile? NewImagePath)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    {
                        category.ImagePath = await FileHelper.UploadFileAsync(NewImagePath, "Categories");
                    }
